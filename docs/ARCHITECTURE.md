...
| `packet.sent` | `connection_id, channel, step_name, packet_bytes, pid, rn` | CommandDispatcher | LogManager |
...
class CmwCommand:
    name: str
    func: Callable[..., Any]          # Функция драйвера для выполнения
    timeout: float = 10.0
    retry_count: int = 3
...
class Cmw500Controller:
    ...
    # Конфигурация:
    async def configure_gsm_signaling(self, **kwargs) -> None
    ...
    async def _execute_with_retry(self, command: CmwCommand, *args) -> Any


class Cmw500Emulator(Cmw500Controller):
    """Полноценный эмулятор CMW-500 для разработки без реального прибора."""
```
...
**Файл:** `core/scenario.py` | **Статус:** ✅ Реализован

 Загрузка и выполнение сценариев из scenario.json через `ScenarioParserFactory`.

Типы шагов (через StepFactory):
 - `expect` — ExpectStep (ожидание пакета с проверкой полей и capture переменных)
 - `send` — SendStep (отправка пакета из файла или build-template)
...
### PacketPipeline + PacketDispatcher + CommandDispatcher

**Файл:** `core/pipeline.py` + `core/dispatcher.py` | **Статус:** ✅ Реализован

 PacketDispatcher и CommandDispatcher — отдельные классы, координирующие pipeline и отправку команд.
...
└──────┬───────┘
       ▼
┌──────────────┐
│ Dedup        │  order=3 → отсев дубликатов по PID из кэша UsvConnection
└──────┬───────┘
       ▼
┌──────────────┐
│ AutoResponse │  order=4 → RESPONSE для успешных + кеш в UsvConnection
└──────┬───────┘
       ▼
┌──────────────┐
...
**Почему Dedup перед AutoResponse:**
- Dedup (order=3) проверяет кеш `_seen_pids` до AutoResponse
- AutoResponse (order=4) формирует RESPONSE и добавляет PID в кеш
- Если бы порядок был обратный — первый пакет помечался как дубликат
...
**Файл:** `core/export.py` | **Статус:** ✅ Реализован
...
TcpServerManager ──emit──► EventBus ◄──emit── Cmw500Controller
   (raw.packet.received)           (raw.packet.received, cmw.error, cmw.connected)
        │                                  │
   ...
   │          PacketDispatcher                       │
   │  _build_pipeline():                          │
   │   CRC → Parse → Dedup → AutoResponse → EventEmit │
   └───────────────────┬────────────────────────┘
...
```
libs/egts/
├── __init__.py                  # Экспорт: Packet, ParseResult, Record, ResponseRecord, ...
├── protocol.py                 # IEgtsProtocol (Protocol)
├── models.py                  # Packet, Record, Subrecord, ParseResult, ResponseRecord (dataclass)
├── types.py                  # Enums: PacketType, ResultCode, ServiceType, SubrecordType
├── registry.py               # Реестр протоколов: get_protocol(version)
│
├── egts/_core/               # Внутренняя реализация (ядро EGTS)
│   ├── parser.py
│   ├── builder.py
│   ├── subrecord.py
│   ├── subrecord_registry.py
│   └── crc.py               # CRC-8/CRC-16
│
└── egts/_gost2015/          # Реализация ГОСТ 33465-2015
    ├── __init__.py           # Регистрация протокола
    ├── protocol.py         # EgtsProtocolGost2015
...
├── core/                          # ЕДИНОЕ ЯДРО
│   ├── __init__.py
│   ├── engine.py                  # CoreEngine (координатор)
...
│   ├── cmw500.py                # Cmw500Controller + Cmw500Emulator + VisaCmw500Driver
│   ├── session.py                # SessionManager + UsvConnection + UsvStateMachine + TransactionManager
...
│   ├── protocol.py             # IEgtsProtocol (Protocol)
│   ├── models.py              # Packet, Record, Subrecord, ParseResult, ResponseRecord
│   ├── types.py              # Enums: PacketType, ResultCode, ServiceType, SubrecordType
│   ├── registry.py           # get_protocol(version)
│   ├── _core/               # Внутренняя реализация
│   │   ├── __init__.py
│   │   ├── parser.py
│   │   ├── builder.py
│   │   ├── crc.py
│   │   ├── subrecord.py
│   │   └── subrecord_registry.py
│   └── _gost2015/            # Реализация ГОСТ 33465-2015
│       ├── __init__.py
│       ├── protocol.py
│       └── subrecords.py
├── cli/                         # CLI приложение
