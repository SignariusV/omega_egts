...
**Обновлено:** 11.04.2026 | **ISSUE-003, ISSUE-004, ISSUE-005, ISSUE-006 решены**
...
## ISSUE-005: `adapter.parse_packet()` не извлекает `ct`, `cid` из COMMAND_DATA SRD

**Статус:** ✅ Решена | **Дата обнаружения:** 11.04.2026 | **Дата решения:** 11.04.2026

| Мета-инфо | Значение |
|-----------|----------|
| **Выявлено на коммите** | `4b6eb67` (debug/manual-testing) — интеграционный тест `test_verification_scenario_full.py` завис на ExpectStep |
| **Решено на коммите** | pending — `adapter.parse_packet()` извлекает `ct`, `cct`, `cid` из COMMAND_DATA SRD → `extra` |
| **Тест обнаружения** | `tests/integration/test_verification_scenario_full.py` — ExpectStep не матчит COMCONF-пакет, timeout 30с |
| **Тест локализации** | Анализ `extra` из EventBus трейсинга: `extra={'service': 4, 'rst': 4, 'subrecord_type': 'EGTS_SR_COMMAND_DATA'}` — нет `ct`, `cid` |
| **Тест подтверждения решения** | `test_verification_scenario_full.py` — 6/6 шагов PASS, `extra_keys=['service', 'rst', 'subrecord_type', 'ct', 'cct', 'cid']` |

### Описание

`ExpectStep._matches()` ищет `ct` и `cid` в `packet_data.extra` для проверки COMCONF-подтверждений (CT_COMCONF=0x01), но `extra` содержит только `service`, `rst`, `subrecord_type`. Сценарий верификации зависает на шаге «Подтверждение APN».

**Подтверждено:** EventBus трейсинг показывает `extra={'service': 4, 'rst': 4, 'subrecord_type': 'EGTS_SR_COMMAND_DATA'}` — SRD-данные COMMAND_DATA не разбираются.

### Хронология появления бага

| Итерация | Что сделано | Где допущена ошибка |
|----------|-------------|---------------------|
| **7.x** | Создан `ExpectStep._matches()` с поддержкой checks по `extra` | ✅ Контракт описан правильно |
| **7.x** | Создан `scenarios/verification/scenario.json` с checks `{"subrecord_type": "CT_COMCONF", "cid": 0}` | ⚠️ Checks написаны для ещё не реализованного парсинга COMMAND_DATA SRD |
| **ISSUE-002** | Решено — `parse_packet()` заполняет `extra` из `records[0]` | ❌ Извлечены только `service`, `subrecord_type` — но НЕ данные SRD |
| **10.0** | Интеграционный тест верификации — **сценарий зависает** | 💥 Баг обнаружен |

### Цепочка данных

```
1. adapter.parse_packet(raw) — COMMAND_DATA пакет (CID=0, CT=1)
   ├── Парсит пакет → packet.records[0].subrecords[0].subrecord_type = "EGTS_SR_COMMAND_DATA"
   ├── data = b'\x10\x00\x00\x00\x00\x00\x00\x00\x00\x00' (SRD COMMAND_DATA)
   └── Возвращает ParseResult(extra={'service': 4, 'rst': 4, 'subrecord_type': 'EGTS_SR_COMMAND_DATA'})
       ← ❌ ct, cid не извлечены из SRD

2. ExpectStep._matches(packet_data.extra)
   ├── Проверяет: extra.get("subrecord_type") == "CT_COMCONF"  ← ❌ "EGTS_SR_COMMAND_DATA" != "CT_COMCONF"
   └── extra.get("cid") → None  ← ❌ cid не извлечён
       └── MATCH FAILED → STEP TIMEOUT
```

### Структура COMMAND_DATA SRD (ГОСТ 33465 таблица 29)

| Смещение | Размер | Поле | Описание |
|----------|--------|------|----------|
| 0 | 1 байт | CT (4 бита) + CCT (4 бита) | Тип команды (0x01=CT_COMCONF) и тип подтверждения |
| 1–4 | 4 байта | CID | Идентификатор команды |
| 5–8 | 4 байта | SID | Идентификатор отправителя |
| 9 | 1 байт | Флаги | AC, CHS и т.д. |

**Реальные данные COMCONF APN:** `\x10\x00\x00\x00\x00\x00\x00\x00\x00\x00`
- `ct_cct = 0x10` → `ct = 1` (CT_COMCONF), `cct = 0` (CC_OK)
- `cid = 0` (из байт 1–4)

### Рекомендуемое решение

**Вариант A (архитектурно правильный):** `adapter.parse_packet()` извлекает данные COMMAND_DATA SRD аналогично RECORD_RESPONSE SRD:
...
**Вариант B (исправить сценарий):** Проверять только `subrecord_type == "EGTS_SR_COMMAND_DATA"` без `ct`/`cid`.

**Рекомендация:** Вариант A — даёт максимальную гибкость сценариям (можно проверять конкретный тип команды).
...
| Файл | Изменение |
|------|-----------|
| `libs/egts_protocol_gost2015/adapter.py` | `parse_packet()` извлекает `ct`, `cct`, `cid` из COMMAND_DATA SRD → `extra` (≥10 байт) |
| `scenarios/verification/scenario.json` | `"subrecord_type": "CT_COMCONF"` → `"subrecord_type": "EGTS_SR_COMMAND_DATA"`, `"ct": 1` (все 3 ExpectStep) |
| `tests/integration/test_verification_scenario_full.py` | Новый интеграционный тест верификации через SMS (6 шагов PASS) |

**Результат после исправления:**

```
[ПАКЕТ] channel=sms, extra_keys=['service', 'rst', 'subrecord_type', 'ct', 'cct', 'cid']
[СЦЕНАРИЙ] Результат: PASS
  Настройка GPRS/APN: PASS
  Подтверждение APN: PASS
  Настройка адреса сервера: PASS
  Подтверждение адреса: PASS
  Запрос UNIT_ID: PASS
  Ответ с UNIT_ID: PASS
```

---

## ISSUE-006: Сценарий верификации содержит некорректные checks для COMCONF

**Статус:** ✅ Решена | **Дата обнаружения:** 11.04.2026 | **Дата решения:** 11.04.2026

| Мета-инфо | Значение |
|-----------|----------|
| **Выявлено на коммите** | `4b6eb67` (debug/manual-testing) — `test_verification_scenario_full.py` FAIL |
| **Решено на коммите** | pending — исправлены checks в `scenarios/verification/scenario.json` |
| **Тест обнаружения** | `tests/integration/test_verification_scenario_full.py` — 3× ExpectStep TIMEOUT |
| **Тест подтверждения решения** | `test_verification_scenario_full.py` — 6/6 шагов PASS |

### Описание

Сценарий верификации (`scenarios/verification/scenario.json`) использует `"subrecord_type": "CT_COMCONF"` в checks, но `CT_COMCONF` — это **тип команды (CT) внутри SRD**, а не тип subrecord. Парсер распознаёт пакет как `"EGTS_SR_COMMAND_DATA"`.
...
{
  "checks": {
    "subrecord_type": "CT_COMCONF",  // ❌ Такой subrecord_type не существует
    "cid": 0                          // ❌ cid не извлекался адаптером (ISSUE-005)
  }
}
```
...
Сценарий верификации **не имел интеграционного теста**. Только auth-сценарий (`scenarios/auth/scenario.json`) проверялся через `test_auth_scenario_full.py`. Верификация осталась без тестового покрытия.
...
| Файл | Изменение |
|------|-----------|
| `scenarios/verification/scenario.json` | Все 3 ExpectStep checks исправлены: `"subrecord_type": "EGTS_SR_COMMAND_DATA"`, добавлен `"ct": 1` (CT_COMCONF), `"cid": 0/1/2` |
| `tests/integration/test_verification_scenario_full.py` | Новый интеграционный тест — эмулятор CMW-500, SMS-обмен, 6 шагов PASS |
