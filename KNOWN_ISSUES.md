...
**Обновлено:** 10.04.2026 | **ТЗ:** v7.0 | **Итерация 5:** Network + CMW-500 (645 тестов, 94% проекта) | **Аудит + исправления:** 12 проблем исправлено (R-066–R-075)
...
|----|----------|------|---------------------|
| CR-008 | **Сессии не получают protocol EGTS при TCP-подключении** | `tcp_server.py:_handle_connection()` вызывает `create_session()` без `protocol` → `conn.protocol = None` → `CrcValidationMiddleware` и `ParseMiddleware` прерывают обработку (`terminated=True`). Весь TCP-трафик отбрасывается. | В `session.py` — автоматическое создание протокола если `protocol is None` (по `gost_version`). В `tcp_server.py` — передача `protocol` в `create_session()`. См. также KI-039 |
...
| CR-010 | **FSM не инициализируется при TCP-подключении** | `conn.fsm.on_connect()` не вызывается → FSM остаётся в `DISCONNECTED`, хотя EventBus эмитит `state="CONNECTED"`. Рассинхронизация: сервер «видит» подключение, FSM — нет. | После `create_session()` вызвать `conn.fsm.on_connect()` и эмитить `connection.changed` с корректным состоянием |
| CR-011 | **FSM не получает on_disconnect при разрыве** | `tcp_server.py:_on_disconnect()` удаляет сессию из dict без вызова `conn.fsm.on_disconnect()` → FSM остаётся в последнем состоянии (например, `RUNNING`), хотя соединение разорвано. | Перед удалением сессии вызвать `conn.fsm.on_disconnect()` и эмитить `connection.changed` с `state="DISCONNECTED"` |
| CR-012 | **FSM не обрабатывает RESPONSE-пакеты (RESULT_CODE)** | `SessionManager._on_packet_processed()` делает ранний возврат при `"service" not in parsed`. RESPONSE-пакеты (RECORD_RESPONSE) не имеют `service` → FSM их игнорирует. Переходы AUTHENTICATING→AUTHORIZED/CONFIGURING требуют `result_code`, который извлекается только из RESPONSE. | Извлекать `result_code` из `ctx.parsed.packet` при типе `EGTS_PT_RECORD_RESPONSE` и вызывать `conn.fsm.on_result_code_sent(result_code)` |
...
| CR-014 | **Неверный порядок middleware в `_build_pipeline()`** | `dispatcher.py:PacketDispatcher._build_pipeline()` задаёт order=3 для AutoResponseMiddleware и order=4 для DuplicateDetectionMiddleware. AutoResponse кеши RESPONSE(pid) в `conn.add_pid_response()` ПЕРЕД тем как Dedup проверяет PID. Результат: первый же пакет определяется как дубликат самого себя. | Поменять order: Dedup=3, AutoResponse=4. Правильный порядок: CRC(1) → Parse(2) → Dedup(3) → AutoResponse(4) → EventEmit(5) |
...
| KI-038 | **Config не используется в компонентах** | `engine.py` — компоненты закомментированы. Параметры (порты, таймауты, версия ГОСТ) захардкожены в конструкторах: `TcpServerManager(port=3001)`, `SessionManager(gost_version="2015")`. | В `CoreEngine.start()` извлекать значения из `self.config` и передавать при создании компонентов |
| KI-039 | **Протокол создаётся без единой фабрики** | `dispatcher.py` создаёт protocol в 2 местах: `_ensure_sms_session()` и `_get_sms_protocol()` — оба через `create_egts_protocol("2015")`. При добавлении ГОСТ 2023 придётся менять все места. | Создать единую фабрику в `SessionManager` или `CoreEngine`, передавать готовый `protocol` во все компоненты |
...
## ✅ Решено в итерации 6.1

| ID | Описание | Решение |
|----|----------|---------|
| R-077 | CR-002: Порядок логов при parallel-обработке | `LogManager` — буферизация + сортировка по timestamp, JSONL-файлы, 23 теста |
...
| ID | Описание | Решение |
|----|----------|---------|
| R-060 | `Cmw500Emulator.send_sms()` минуется очередь команд | Перенесено в `_handle_send_sms()` → `_send_scpi()`, теперь идёт через worker_queue |
| R-061 | `_incoming_sms_queue.task_done()` без `join()` | Убран из эмулятора — `task_done()` не нужен без `join()` |
| R-062 | Fire-and-forget `create_task()` при закрытом event loop | Обёрнуто в `try/except RuntimeError` в `_on_worker_done` и `_on_poll_done` |
| R-063 | Async handler кладёт coroutine object в очередь вместо bytes | Исправлено: `handler_result = await handler_result` сохраняет результат |
| R-064 | PacketDispatcher без тестов | 24 теста, 95% coverage |
| R-065 | CommandDispatcher без тестов | 23 теста, 95% coverage (совместно с PacketDispatcher) |
| R-066 | `SessionManager._on_packet_processed` — ParseResult → `parsed = {}`, FSM теряет `service` | Исправлено: извлечение `service_type` из `packet.records[0]` и данных из `subrecords[].data` |
| R-067 | `CommandDispatcher._send_sms` — SMS-транзакция не регистрировалась без существующей сессии | Добавлен `_ensure_sms_session_for_txn()` — создаёт сессию при необходимости |
| R-068 | `Cmw500Controller.execute()` — future мог зависнуть при отключении worker | `asyncio.get_event_loop()` заменён на `get_running_loop()`; таймаут через `wait_for` отложен (конфликт с тестами retry) |
| R-069 | `TcpServerManager.stop()` — race condition: задачи отменялись до закрытия сервера | Исправлено: snapshot задач `list(self._tasks)` + порядок cancel→gather→close→wait_closed |
| R-070 | `asyncio.get_event_loop()` — deprecated в Python 3.12+ | Заменено на `asyncio.get_running_loop()` в `_on_worker_done` и `_on_poll_done` |
| R-071 | `_execute_with_retry` — `retry_count=0` вызывал `TypeError: raise None` | Добавлена проверка `if last_error is not None` |
| R-072 | Дублирование логики `is_closing` в dispatcher.py | Вынесено в утилиту `_is_writer_closing(writer)` |
| R-073 | Lazy import `create_protocol` внутри методов dispatcher | Перенесён на верхний уровень модуля |
| R-074 | `_default_protocol` в тестах — мёртвый код | Удалено — SessionManager создаёт protocol из `gost_version` |
| R-075 | `test_start_emits_server_started` — не проверял событие | Теперь подписывается на `server.started` и верифицирует порт |
| R-076 | CR-009: Отсутствие AutoResponseMiddleware | Добавлен `AutoResponseMiddleware` (order=3), `_build_pipeline()` в PacketDispatcher, 12 тестов, 96% coverage pipeline.py |
