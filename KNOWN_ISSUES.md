...
| KI-063 | GUI не может остановить выполняющийся сценарий | Открыто | `CoreEngine` не имеет метода `stop_scenario()`. `ScenarioManager.execute()` выполняется синхронно без механизма отмены. При нажатии Stop в GUI: `'EngineWrapper' object has no attribute 'stop_scenario'`. Решение: добавить асинхронную отмену через `asyncio.Task` + флаг прерывания между шагами |
| KI-064 | **`EngineWrapper.stop_scenario()` не реализован** | Частично | ~~`engine_wrapper.py:53` вызывал `self.engine.stop_scenario()`, которого нет в `CoreEngine`~~. Исправлено: `EngineWrapper.stop_scenario()` теперь raises `NotImplementedError` с понятным сообщением вместо `AttributeError`. Полная реализация требует механизма отмены в `ScenarioManager` (KI-063) |
