...
| CR-009 | **Отсутствует AutoResponseMiddleware** | Нет middleware, формирующего `ctx.response_data` для корректных пакетов. RESPONSE отправляется только при ошибках CRC и дубликатах. По ГОСТ 33465 сервер обязан отвечать на каждый пакет → УСВ повторяет пакеты, теряет синхронизацию. | Добавить `AutoResponseMiddleware` в pipeline: после успешного парсинга формирует `RECORD_RESPONSE` (для записей) или `RESPONSE` (для пустых пакетов) |
