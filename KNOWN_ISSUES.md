...
**Обновлено:** 11.04.2026 | **ТЗ:** v7.0 | **Итерации 1–12:** 1000+ тестов | **Аудит + исправления:** R-001–R-100
...
|----|----------|--------|---------------------|
| KI-060 | **Некорректная реализация read_sms в Cmw500Controller** | Открыто | В текущей реализации метод `read_sms` возвращает строку hex-данных вместо байтов. Это приводит к ошибке `ValueError: non-hexadecimal number found in fromhex() arg at position 0` при попытке преобразовать данные в байты. Необходимо исправить метод `read_sms` для корректного преобразования hex-строки в байты с использованием `bytes.fromhex()` |
...
| ID | Описание | Статус | Планируемое решение |
|----|----------|--------|---------------------|
| KI-041 | `LogManager.stop()` не вызывается в `_cleanup()` | Открыто | Добавить `await self.log_mgr.stop()` перед `self.log_mgr = None` в `_cleanup()`. Это flush-ит буфер, отменяет auto-flush задачу и отписывается от EventBus |
