...
| CR-004 | `crcmod` — внешняя зависимость для CRC | Требуется `pip install crcmod`; может отсутствовать в некоторых средах | Добавить в `pyproject.toml`; предусмотреть fallback-реализацию на чистом Python |
...
| KI-008 | `egts_protocol_gost2015/adapter.py` — заглушка | Методы бросают `NotImplementedError`; парсинг/сборка не реализованы | Этап 2.3–2.4: копирование из EGTS_GUI, адаптация под IEgtsProtocol |
...
- **egts_protocol_gost2015**: Заглушка (адаптер). Полная реализация — этап 2.3–2.4 (копирование парсера из EGTS_GUI).
