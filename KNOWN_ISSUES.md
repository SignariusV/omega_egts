...
**Обновлено:** 10.04.2026 | **ТЗ:** v7.0 | **Итерации 1–8:** 870 тестов, 90%+ проекта | **Аудит + исправления:** R-001–R-100
...
| CR-014 | ~~**Неверный порядок middleware в `_build_pipeline()`**~~ | ~~AutoResponse(order=3) перед Dedup(order=4) → дубликаты~~ | **Решено (R-084)**: Порядок исправлен: CRC(1) → Parse(2) → AutoResponse(3) → Dedup(4) → EventEmit(5). AutoResponse кеширует RESPONSE, Dedup находит дубликат из кэша |
...
| R-084 | CR-014: Неверный порядок middleware | Порядок исправлен: CRC(1) → Parse(2) → AutoResponse(3) → Dedup(4) → EventEmit(5) | 5.4 |
