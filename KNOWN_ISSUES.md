...
| R-050 | `session.py` — `_last_transition` — доступ к приватному атрибуту из SessionManager | Добавлен публичный `@property last_transition` в UsvStateMachine | 7.0 |
| R-051 | `session.py` — `cleanup_expired` утекает `_by_rn` записи (только rn, без pid) | Добавлена итерация по `_by_rn` для orphan-записей + метод `_remove_txn` | 7.0 |
| R-052 | `session.py` — `contextlib.suppress(Exception)` слишком широк в `close_session` | Заменено на `suppress(asyncio.TimeoutError, OSError)` | 7.0 |
| R-053 | `session.py` — `create_session` молча перезаписывала дубликат connection_id | Добавлена проверка: `ValueError` при дубликате | 7.0 |
| R-054 | `session.py` — `_on_packet_processed` обрабатывал пустой `{}` без service | Добавлен ранний возврат при `"service" not in parsed` | 7.0 |
| R-055 | `session.py` — `bus: Any` вместо конкретного типа в SessionManager | Заменено на `bus: EventBus` | 7.0 |
| R-056 | `session.py` — дублирование логики удаления в TransactionManager | Выделен метод `_remove_txn(txn)` | 7.0 |
| R-057 | `session.py` — `on_timeout` увеличивал счётчик для DISCONNECTED/ERROR | Добавлен ранний возврат для терминальных состояний | 7.0 |
| R-058 | `session.py` — `@state.setter` нарушал инкапсуляцию FSM | Удалён, состояние меняется только через `_transition()` | 7.0 |
| R-059 | `session.py` — `_handle_authenticating` не сбрасывал счётчик при AUTH_INFO/RECORD_RESPONSE | Добавлен `self._timeout_counter = 0` + обработка RST != 0 | 7.0 |
