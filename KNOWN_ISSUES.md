...
| KI-048 | `get_status()` — redundant `and self.is_running` | Открыто | `_cleanup()` всегда ставит `cmw500 = None` до изменения `_started`. Проверка `cmw500 is not None` достаточна |
 ...
 | ~~KI-050~~ | ~~Относительный `log_dir` — риск при смене cwd~~ | ~~Открыто~~ | **Решено**: CLI и GUI теперь используют абсолютный путь `BASE_DIR / "logs"` к корневой директории проекта. `get_log_dir()` корректно сохраняет и возвращает путь. |
...
| KI-062 | CMW-500 модуль может препятствовать запуску | Открыто | Если в конфигурации указан IP CMW-500, но модуль RsCmwGsmSig не установлен, запуск завершается ошибкой до инициализации LogManager. Решение: `cmw500.ip = None` для работы без CMW-500 |
 ...
 | KI-064 | MCC/MNC захардкожены как 250/77 | По ТЗ п. 2.1.2 в): NID=25077 не корректируемо. Валидация в `_validate_cmw()` вызывает ValueError при mcc≠250 или mnc≠77. Если потребуется поддержка других операторов — потребуется изменение ТЗ или отдельный режим. |
 | KI-065 | SRT коды в `SubrecordType` не совпадают с номерами из ГОСТ | Перечисление используется только для читаемости, парсеры регистрируются по числовому `srt`. Требуется верификация значений |
 | KI-066 | COMMAND_CODES 0x0501/0x0502 (MIC/SPK level) добавлены на основе контекста ТЗ | Требуют верификации по ГОСТ 33465-2015 Таблица 34 |
 | KI-067 | GUI-тесты требуют PyQt/PySide окружения | `tests/gui/` пропускаются в CI без GUI-зависимостей |
