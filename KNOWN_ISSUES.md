...
**Обновлено:** 23.04.2026 | **ТЗ:** v7.0 | **Итерации 1–14:** 1000+ тестов | **Аудит + исправления:** R-001–R-100
...
| ~~CR-013~~ | ~~**Дублирующее создание SMS-сессии**~~ | ~~`PacketDispatcher._ensure_sms_session()` и `CommandDispatcher._ensure_sms_session_for_txn()` создают сессию независимо → potential race condition при одновременном вызове. Протокол создаётся в двух местах через `create_egts_protocol("2015")`.~~ | **Решено (R-100)**: `SessionManager.get_or_create_session()` + `ensure_sms_session()` — единая фабрика. Оба диспетчера вызывают `session_mgr.ensure_sms_session()`, протокол из `self.gost_version`. Удалены `_ensure_sms_session()`, `_ensure_sms_session_for_txn()`, `_get_sms_protocol()`. 6 новых тестов. 441/443 PASS. |
...
| ~~CR-005~~ | ~~Параллельные агенты перезаписывают `adapter.py`~~ | ~~Задачи 2.3 и 2.4 редактируют один файл — SMS-методы затираются~~ | **Неактуально**: После миграции на libs/egts/ (541fb1e) всё в разных модулях |
...
| ~~KI-039~~ | ~~**Повторное создание SMS-сессии в двух местах**~~ | ~~Открыто~~ | **Решено совместно с CR-013 (R-100)**: единая `SessionManager.ensure_sms_session()`. |
...
| ~~KI-047~~ | ~~`ScenarioManager.execute()` не эмитит `scenario.step` в EventBus~~ | ~~Открыто~~ | **Решено**: Добавлен `await bus.emit("scenario.step", ...)` после каждого шага + `scenario.started` в начале + `scenario.finished` в конце. GUI получает полный список шагов и обновляет таблицу/прогресс в реальном времени. |
...
| KI-061 | LogManager буферизует записи и не записывает на диск немедленно | Открыто | При аварийном завершении программы логи могут быть потеряны. Flush происходит: по порогу (1000 записей), по интервалу (5 сек), при остановке engine. Для отладки использовать принудительный flush. |
| KI-062 | CMW-500 модуль может препятствовать запуску | Открыто | Если в конфигурации указан IP CMW-500, но модуль RsCmwGsmSig не установлен, запуск завершается ошибкой до инициализации LogManager. Решение: `cmw500.ip = None` для работы без CMW-500 |
| ~~KI-063~~ | ~~GUI не может остановить выполняющийся сценарий~~ | ~~Открыто~~ | **Решено**: `ScenarioManager` имеет `cancel()` (флаг `_cancel_requested`), `CoreEngine` запускает `run_scenario()` как background task (`_scenario_task`), `cancel_scenario()` отменяет task + флаг. GUI кнопка Stop работает. |
| ~~KI-064~~ | ~~**`EngineWrapper.stop_scenario()` не реализован**~~ | ~~Частично~~ | **Решено**: `stop_scenario()` делегирует в `engine.cancel_scenario()` — отменяет background task и флаг `_cancel_requested`. |
| KI-065 | **`EngineWrapper.start()` не проверяет `is_running`** | Открыто | Повторный вызов `EngineWrapper.start()` повторно выполняет проверку порта (может дать false negative если TcpServerManager уже забиндил сокет) и вызывает `CoreEngine.start()` (который игнорирует повторный вызов). Добавить `if self.engine.is_running: return` в начало `start()` |
| KI-066 | **`EngineWrapper.load_scenario_info()` дублирует создание парсера** | Открыто | Каждый вызов создаёт новые `ScenarioParserRegistry`, `ScenarioParserFactory`, `ScenarioParserV1`. В `CoreEngine.start()` та же логика. Вынести в общий модуль или принимать factory через `__init__` |
| KI-067 | **`EventBridge` не отписывается от EventBus** | **Решено** | Добавлен метод `EventBridge.unsubscribe()`, вызывается в `main_window.closeEvent()`. Предотвращает memory leak и crash при закрытии окна |
| KI-068 | **`EventBridge.cmw_error` теряет контекст** | **Решено** | Сигнал изменён с `Signal(str)` на `Signal(dict)`. Теперь передаёт полный dict с полями `error`, `command` и др. |
...
| ~~KI-052~~ | ~~**Transaction registration ПОСЛЕ отправки** (`dispatcher.py:436-444`)~~ | ~~`writer.write()` + `drain()` происходит ДО регистрации транзакции. Если connection drop во время drain — transaction теряется. Переместить `transaction_mgr.register()` перед `writer.write()`~~ | **Решено (18.04.2026)**: `register()` перемещён перед `write()` + `drain()` |
| ~~KI-053~~ | ~~**ResponseRecord rn vs CRN путаница** (`pipeline.py:371-380`)~~ | ~~`ResponseRecord(rn=rec.record_id)` — rn используется и как RN record'а, и как CRN в subrecord data. Нужна верификация против adapter.py, что сериализация корректна~~ | **Проверено (18.04.2026)**: Код корректен — RN записи подтверждается через CRN в subrecord (по ГОСТ) |
| ~~KI-054~~ | ~~**Unbounded buffer в LogManager** (`logger.py:103-117`)~~ | ~~`_auto_flush_loop` flush раз в интервал, но буфер растёт быстрее при высокой нагрузке. Использовать `while` loop для полной очистки буфера~~ | **Решено (18.04.2026)**: Добавлен `while self._buffer: await self.flush()` |
...
| ~~KI-057~~ | ~~**Mixed int/str comparison для subrecord_type**~~ | ~~Открыто~~ | **Решено (R-095)**: Поддержка обоих форматов для обратной совместимости |
...
Сейчас сценарии используют JSON, который не поддерживает комментарии и требует числовых значений для полей EGTS (например, `"ct": 5` вместо `"ct": "COM"`).

**Зачем переходить на YAML:**
- **Комментарии** — можно добавлять пояснения к каждому шагу и полю прямо в файле сценария
- **Читаемость** — меньше скобок, более чистая структура
- **Строковые алиасы** — можно использовать `"ct": "COM"`, `"ccd": "EGTS_GPRS_APN"` вместо чисел

**Пример сценария в YAML:**
```yaml
# Сценарий настройки УСВ через SMS
name: "Верификация (SMS)"
scenario_version: "1"

steps:
  - name: "Настройка GPRS/APN"
    type: send
    channel: sms
    build:
      gost_version: "2015"
      packet:
        packet_id: 27
        packet_type: APPDATA  # 1 — пакет данных приложения
        records:
          - record_id: 42
            service_type: COMMANDS  # 4 — сервис команд
            rsod: true
            subrecords:
              - subrecord_type: COMMAND_DATA  # 51
                data:
                  ct: COM       # 5 — команда для выполнения
                  cct: OK       # 0 — ожидание подтверждения
                  cid: 0
                  cd:
                    adr: 0
                    act: SET    # 2 — установка значения
                    ccd: EGTS_GPRS_APN  # 257
                    dt: "internet"
```

**Что потребуется для реализации:**
1. Добавить `pyyaml` в зависимости
2. Создать `ScenarioYamlParser` или расширить `ScenarioManager.load()` для автодетекта формата (`.json` vs `.yaml`)
3. Реализовать резолвер строковых алиасов (конвертация `"COM"` → `5`, `"EGTS_GPRS_APN"` → `257`)
4. Конвертировать существующие `.json` сценарии в `.yaml`

**Обратная совместимость:** JSON-сценарии продолжат работать, YAML будет опциональным форматом.
