...
> ✅ **Ядро EGTS-протокола реализовано.** Библиотека `egts_protocol_gost2015` готова к использованию:
> парсинг/сборка пакетов, CRC-8/16, SMS PDU, сервисы (auth, commands, ecall, firmware).
>
> Следующий этап: FSM авторизации + TCP-сервер (Этап 1) + сценарии (Этап 3).
...
|------|-----------|--------|
| **1. CLI MVP** | Core Engine + EventBus + FSM + CLI + ГОСТ 2015 + эмулятор CMW-500 | 🟡 В работе (50%) |
| **2. Библиотека EGTS** | egts_protocol_iface + EgtsProtocol2015 (парсинг/сборка/SMS) | ✅ Готово (100%) |
| **3. Сценарии** | Все 10 сценариев, HEX-файлы, export, CredentialsRepository | ⬜ Запланировано |
| **4. ГОСТ 2023** | Поддержка актуальной редакции | ⬜ Запланировано |
| **5. GUI** | Графический интерфейс оператора | ⬜ Запланировано |
| **6. Реальное CMW-500** | Интеграция с реальным оборудованием | ⬜ Запланировано |
