...
#### Changed
- **`core/session.py:487-490`** — закомментирована проверка `ValueError` при дубликате PID/RN в `TransactionManager.register()`. Теперь регистрация перезаписывает старую транзакцию новым PID без ошибки.

#### Причина
В hex-файлах пакетов `gprs_apn.hex` и `server_address.hex` (и в динамических сценариях с `{{packet_id}}`) используется одинаковый PID=27, из-за чего второй SendStep падал с `ValueError: Дубликат PID: 27`. Временное решение — отключение проверки до реализации автоинкремента PID.

#### KNOWN_ISSUES
- **KI-075**: Проверка дубликатов PID отключена — transaction_mgr не защищает от повторного использования PID. См. KNOWN_ISSUES.md
